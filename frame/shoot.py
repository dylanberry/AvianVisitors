...
Pi Zero W. Writes a 1200x1600 PNG; display.py turns it into panel pixels.
  ...
  python3 shoot.py --url https://birds.spadaberry.net \
      --title "Bird Up!" --subtitle "heard today" --out frame.png
"""
...
RAW_ILLUSTRATIONS = (
    "https://raw.githubusercontent.com/dylanberry/BirdUp/"
...
def _frame_css(
    headline_px, eyebrow_px, lowercase, pad_top, pad_side, pad_bottom, collage_vh
):
    css = (
    ...
    def handler(route):
        ...
        try:
            if species is not None:
                ...
                url = (
                    re.sub(r"hours=\d+", f"hours={int(window_hours)}", req.url)
                    if window_hours
                    else req.url
                )
                kw = {"url": url}
...
def _make_cutout_handler(base, local_dir=None):
    ...
    def handler(route):
        try:
            ...
            slug = re.sub(
                r"[^a-z0-9]+", "-", (params.get("sci") or [""])[0].lower()
            ).strip("-")
...
def _make_js_handler(xbias, ybias, count_exp, pad, auth, misses):
    ...
    def handler(route):
        try:
            kw = (
                {"headers": {**route.request.headers, "authorization": auth}}
                if auth
                else {}
            )
            ...
            for pat, repl in (
                (
                    r"var xBias = narrow \? 1 : T\.ellipseAspectBias;",
                    f"var xBias = {xbias};",
                ),
                (r"var yBias = narrow \? 1\.7 : 1;", f"var yBias = {ybias};"),
                (r"countExp:\s*[\d.]+,", f"countExp: {count_exp},"),
                (
                    r"var pad = narrow \? Math\.max\(1, COLLAGE_PAD - 1\) : COLLAGE_PAD;",
                    f"var pad = {pad};",
                ),
            ):
...
def shoot(
    ...
    *,
    ...
    cutout_base=None,
    cutout_local=None,
    empty_text="listening for birds…",
):
    pad_side, pad_top, pad_bottom = int(vw * mat), int(vh * mat * 0.92), int(vh * mat)
    auth = (
        "Basic " + base64.b64encode(f"{user}:{password or ''}".encode()).decode()
        if user
        else None
    )
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.route(
                "**/birdnet-api.php**",
                _make_api_handler(small_floor, window_hours, auth, species),
            )
            page.route(
                "**/apt.js*",
                _make_js_handler(
                    cluster_xbias, cluster_ybias, count_exp, cluster_pad, auth, misses
                ),
            )
            if cutout_base:
                page.route(
                    "**/cutout.php*", _make_cutout_handler(cutout_base, cutout_local)
                )
            ...
            if resp is None or not resp.ok:
                raise RuntimeError(
                    f"site returned {resp.status if resp else 'no response'}"
                )
            ...
            page.wait_for_selector(
                ".gtile, .empty", state="attached", timeout=timeout_ms
            )
            if page.query_selector(".gtile") is not None:
                try:
                    ...
                    print(
                        "some illustrations did not finish loading; capturing anyway",
                        file=sys.stderr,
                    )
                ...
                try:
                    ...
                    print(
                        "nest illustration did not finish loading; capturing anyway",
                        file=sys.stderr,
                    )
            if misses:
                raise RuntimeError(
                    f"apt.js tunables not found ({len(misses)}); refusing to ship a half-tuned frame"
                )

            if title is not None:
                page.evaluate(
                    "t=>{const e=document.querySelector('.static-head .pre'); if(e)e.textContent=t;}",
                    title,
                )
            if subtitle is not None:
                page.evaluate(
                    "s=>{const e=document.querySelector('.static-head h1'); if(e)e.textContent=s;}",
                    subtitle,
                )
            ...
            page.evaluate(
                "(t) => { const e = document.querySelector('.empty'); if (e) { e.textContent = t; e.style.color = '#555'; } }",
                empty_text,
            )
...
def shoot_birdweather(
    out, species, *, title=None, subtitle=None, timeout_ms=45000, **look
):
...
def main():
    ap = argparse.ArgumentParser(
        description="Screenshot the Bird Up! collage for the e-ink frame."
    )
    ...
    ap.add_argument(
        "--bw-days", type=int, default=7, help="--bird-weather lookback window in days"
    )
    ap.add_argument(
        "--bw-country", default="us", help="--bird-weather geocoder country code"
    )
    ...
    if a.bird_weather:
        ...
        species = birdweather.species_for_zip(
            a.zip, country=a.bw_country, days=a.bw_days
        )
        if not species:
        ...
        look = {
            k: v
            for k, v in (
                ("count_exp", a.count_exp),
                ("headline_px", a.headline_px),
                ("eyebrow_px", a.eyebrow_px),
            )
            if v is not None
        }
        look.update(
            ...
            dsf=a.dsf,
            mat=a.mat,
            collage_vh=a.collage_vh,
            cluster_xbias=a.cluster_xbias,
            cluster_ybias=a.cluster_ybias,
            cluster_pad=a.cluster_pad,
