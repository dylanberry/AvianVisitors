...
def _graphql(query, timeout):
    req = urllib.request.Request(
        ...
        headers={"Content-Type": "application/json", "User-Agent": "BirdUp-frame/1.0"},
    )
    try:
...
def ebird_nearby(lat, lon, days=14, key=None, timeout=20):
    ...
    req = urllib.request.Request(url, headers={"X-eBirdApiToken": key, "User-Agent": "BirdUp-frame/1.0"})
    try:
        with urllib.request.urlopen(req, timeout=timeout) as r:
